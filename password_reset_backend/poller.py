...
<SessionID>{{ ticket }}</SessionID>
            ...
            if type(unwrapped_request) is dict:
                if 'type' in unwrapped_request and 'id' in unwrapped_request and 'request_content' in unwrapped_request:
                    ...
                    if request_type == 'checkauthcode':
                        ...
                        if not verified:
                            requests.post('%s/checkcoderesponse/%s/Failed' % (self.frontend_addr, id),
                                          data=json.dumps(
                                              {'status': 'Failed', 'message': 'The reset code supplied was incorrect'}))
                        else:
                            requests.post('%s/checkcoderesponse/%s/OK' % (self.frontend_addr, id),
                                          data=json.dumps(
                                              {'status': 'OK'}))
                    ...
                    if request_type == 'checknameexist':
                        ...
                        if user is None:
                            ...
                            if (uid == user_id):
                                requests.post('%s/checknameresponse/%s/OK' % (self.frontend_addr, id),
                    ...
                    if request_type == 'resetwithsmartcard':
                        ...
                        try:
                            ...
                            requests.post('%s/resetresponse/%s/OK' % (self.frontend_addr, id),
                                          data=json.dumps({'status': 'OK'}))
    ...
    def send_code(self, username, id):
        ...
        if users is None or len(users) == 0:
            ...
            user = users[0]
            pager = []
            if user['pager'] is not None and type(user['pager']) is tuple:
                pager = user['pager'][0].split(':')
            elif user['pager'] is not None:
    ...
    def set_user_details(self, reset_request):
        if 'id' in reset_request \
            ...
            if conn.result['description'] == 'success':
                ...
                self.log.warning('Method=set_user_details, Message=Other error occurred2, Result=%s, Request=%s' % (conn.result, reset_request))
                requests.post('%s/setuserdetails/%s/Failed' % (self.frontend_addr, id),
    ...
    def reset_ad_password(self, username, new_password=None):
        ...
        try:
            ...
            pwd.set(user['distinguishedName'], username, self.domain_fqdn, password=new_password)
            ...
            self.log.exception('Method=reset_ad_password, Message=Failed to set password in AD, Username=%s' % user['distinguishedName'])
            raise(ex)
